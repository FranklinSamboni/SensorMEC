...
perror("fopen error in brsac:");
	...
	*nerr = 0;
	...
	fread(*data,sizeof(float),maxpts,fptr);
	fclose (fptr) ;
	/* safety */
	if(sachdr.ihdr[9] > LN){
			maxpts = LN;
...
perror("fopen error in bwsac:");
	...
	fwrite(data,sizeof(float),LN,fptr);
	fclose (fptr) ;
}
...
void createFile(char *name){
	FILE *fptr;
	if((fptr=fopen(name,"wb")) == NULL){
		perror("fopen error in createFile:");
		return;
	}
	#ifdef MSDOS
		setmode(fileno(fptr), O_BINARY);
...
void updateHeaders(char *name){
	...
	if((fptr=fopen(name,"r+")) == NULL){
		perror("fopen error in updateHeaders:");
		return;
	}
	#ifdef MSDOS
		setmode(fileno(fptr), O_BINARY);
...
void updateData(char *name,int LN,float *data){
	...
	if((fptr=fopen(name,"a")) == NULL){
		perror("fopen error in updateData:");
		return;
	}
	#ifdef MSDOS
		setmode(fileno(fptr), O_BINARY);
...
void scmxmn(float *x, int npts, float *depmax, float *depmin, float *depmen)
{
	...
	float depmaxInit = -1.0e+38 ;
	float depminInit = 1.0e+38 ;
	/**depmax =
	*depmin =  */
	sum = 0.0e+00 ;
	for( i=0 ; i < npts ; i++){
		if( x[i] > depmaxInit) depmaxInit = x[i] ;
		if( x[i] < depminInit) depminInit = x[i] ;
		sum +=  x[i] ;
 	}
	if(npts > 0){
		*depmen = sum / npts ;
	...
	if( *depmax == 0.0 && *depmin == 0.0 ){
		printf("depmax - depmin son nulos, se van a inicializar.\n");
		*depmax = depmaxInit;
		*depmin = depminInit;
	}
	else{
		if(depmaxInit > *depmax ){
			*depmax = depmaxInit;
		}
		if(depminInit < *depmin){
			*depmin = depminInit;
		}
	}
}
